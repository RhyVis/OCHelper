...
public class DataProcessor {
    ...
    public Pair<List<AeReportItemObj>[], AeReportItemObj> requestAeCpuDetail(int cpuid) {
        ...
        cpuDetailIndex = 0;
        var cpl = List.of(CommandPackEnum.AE_GET_CPU_DETAIL_ACTIVE.ofParams(cpuid),
    ...
    public List<String> requestGtMachineSensor(String proxyAddress) {
        ...
        boolean timeout = false;
        var cc =
            CommandPackEnum.GT_GET_SENSOR.ofParams(proxyAddress);
        lock.lock();
        try {
            ls.injectMission(cc);
                ...
                switch (k) {
                    ...
                    case "GT_GET_ENERGY_WIRELESS" -> {
                        if (!error) {
                            ...
                            log.error("Caught error in GT_GET_ENERGY_WIRELESS, energy station connection may not"
                                + " set properly.");
                        }
                    }
