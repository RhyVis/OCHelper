...
public class DatabaseAccessor {
    ...
    private final String requestAllSql =
            "SELECT name FROM sqlite_master WHERE type='table' ORDER BY name;";

    private AeDataSetObj getAeItemDataObjLatest(String un) {
        return idt.query(
                        sf.generateSelectLatest(un),
                        (rs, rowNum) ->
                                AeDataSetObj.of(
                                        un, rs.getLong(1), rs.getString(2), rs.getString(3)))
                .getFirst();
    }
    ...
    public List<AeDataSetObj> getAeItemDataObjN(String un, int size) {
        return idt.query(
                sf.generateSelect(un, size),
                (rs, rowNum) ->
                        AeDataSetObj.of(un, rs.getLong(1), rs.getString(2), rs.getString(3)));
    }
    ...
    public List<EnergyData> getEnergyWirelessDataN(int size) {
        return edt.query(
                sf.generateEnergyWirelessDataSelect(size),
                (rs, rowNum) -> EnergyData.of(rs.getLong(1), rs.getString(2), rs.getString(3)));
    }
}
