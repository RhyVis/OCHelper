...
public class DatabaseAccessor {
    ...
    private final @Qualifier("itemJdbcTemplate") JdbcTemplate idt;
    private final @Qualifier("fluidJdbcTemplate") JdbcTemplate fdt;
    private final @Qualifier("energyJdbcTemplate") JdbcTemplate edt;
    ...
    private AeDataSetObj getAeItemDataObjLatest(String un) {
        return idt.query(sf.generateSelectLatest(un), (rs, rowNum) -> AeDataSetObj.builder().un(un).id(rs.getLong(1))
            .size(rs.getString(2)).time(rs.getString(3)).build()).getFirst();
    }

    private AeDataSetObj getAeFluidDataObjLatest(String un) {
        return fdt.query(sf.generateSelectLatest(un), (rs, rowNum) -> AeDataSetObj.builder().un(un).id(rs.getLong(1))
            .size(rs.getString(2)).time(rs.getString(3)).build()).getFirst();
    }
    ...
    public List<AeDataSetObj> getAeItemDataObjN(String un, int size) {
        return idt.query(sf.generateSelect(un, size), (rs, rowNum) -> AeDataSetObj.builder().un(un).id(rs.getLong(1))
            .size(rs.getString(2)).time(rs.getString(3)).build());
    }

    public List<AeDataSetObj> getAeFluidDataObjN(String un, int size) {
        return fdt.query(sf.generateSelect(un, size), (rs, rowNum) -> AeDataSetObj.builder().un(un).id(rs.getLong(1))
            .size(rs.getString(2)).time(rs.getString(3)).build());
    }
