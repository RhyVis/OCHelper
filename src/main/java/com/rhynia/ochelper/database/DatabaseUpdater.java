...
public class DatabaseUpdater {
    ...
    private void initCleanDatabaseItem() {
        var tmp =
                idt.queryForList(requestAllSql, String.class).stream()
                        .map(sf::generateClean)
                        .toArray(String[]::new);
        if (!(tmp.length == 0)) {
            idt.batchUpdate(tmp);
        }
    ...
    private void initCleanDatabaseFluid() {
        var tmp =
                fdt.queryForList(requestAllSql, String.class).stream()
                        .map(sf::generateClean)
                        .toArray(String[]::new);
        if (!(tmp.length == 0)) {
            fdt.batchUpdate(tmp);
    ...
    public void updateItemDatabase(List<AeReportItemObj> itemList) {
        ...
        var tmp =
                Stream.concat(
                                itemList.stream().map(sf::generateCheck),
                                itemList.stream().map(sf::generateInsert))
                        .toArray(String[]::new);
        idt.batchUpdate(tmp);
    }
    ...
    public void updateFluidDatabase(List<AeReportFluidObj> fluidList) {
        ...
        var tmp =
                Stream.concat(
                                fluidList.stream().map(sf::generateCheck),
                                fluidList.stream().map(sf::generateInsert))
                        .toArray(String[]::new);
        fdt.batchUpdate(tmp);
    }
