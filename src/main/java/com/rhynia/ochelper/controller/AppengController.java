...
public class AppengController {
    ...
    public String transportItemData(String un, String latest, Model model) {
        ...
        var processed = original.stream().sorted(Comparator.comparingLong(AeDataSetObj::getId).reversed())
    ...
    public String requestCraftables(Model model) {
        ...
        var list =
            dp.requestAeCraftList().stream().peek(obj -> obj.setImgPath(imgPath + obj.getLocal() + ".png")).toList();
    ...
    public String requestCraft(@RequestParam("amount") int amount, @RequestParam("name") String name,
        @RequestParam("meta") int meta, Model model) {

        var local = Format.tryTranslateItemUn(Format.assembleItemUniqueName(name, meta));
        log.info("Received crafting request of '{}:{}' with amount of {}, its local may be {}.", name, meta, amount,
            local);
